...
let autoMove = function(){
    ...
    // 设置slider联动
    handleSlider(previousIndex,currentIndex);

    previousIndex = currentIndex;
};

// 切花slider显示效果
let handleSlider = function(previousIndex,currentIndex){
    utils.CSS(sliderList[currentIndex > sliderList.length - 1 ? 0 : currentIndex], {
        background: 'red'
    });
    utils.CSS(sliderList[previousIndex > sliderList.length - 1 ? 0 : previousIndex], {
        background: 'white'
    });
};
...
let sliderClick = function(){
    ...
    [].forEach.call(sliderList,(item,index)=>{
        ...
        item.onclick = ()=>{
            ...
            handleSlider(previousIndex,index);
            currentIndex = index;
            previousIndex = currentIndex;
        };
    });
